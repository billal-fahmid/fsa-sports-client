    ...
    const handleDelete = (_id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(`https://server-rouge-gamma.vercel.app/selectedclasses/${_id}`, {
                    method: 'DELETE'
                })
                    .then(res => res.json())
                    .then(data => {
                    ...
                    <tbody>
                        {
                            selectedClasses?.map((item, index) => <tr key={item._id} className="hover">
                                ...
                                <td> <Link to={`/dashboard/payment/${item._id}`} className="btn btn-outline btn-sm btn-info">Pay Now</Link></td>
